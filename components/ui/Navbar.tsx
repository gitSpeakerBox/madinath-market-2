...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`main_padding_x flex  items-center justify-between lg:justify-around bg-[#082801] py-[14px] z-[999]  `}
      >
        <Link href={"/"} className="outline-none bg-none cursor-pointer">
          <Logo className="w-[120px] h-[40px] lg:w-[140px] lg:h-[47px]" />
        </Link>
        <div className="flex gap-[60px] items-center max-lg:hidden">
          {/* {NavLinks.map((val, idx) => (
            <Link
              className="font-medium text-sm capitalize outline-none lg:hover:text-mm-green text-white rounded-[3px] duration-300"
              key={val.href}
              href={val.href}
            >
              {val.title}
          ...
          {NavLinks.map((val, idx) =>
            val.subLinks ? (
              <div
                className="font-medium group z-50 relative text-white text-sm capitalize outline-none  lg:hover:text-mm-green rounded-[3px] duration-300"
                key={val.href}
              >
                {val.title}
                <motion.div
                  ...
                  className={`absolute z-50 flex-col bg-[#082801]  p-4 hidden group-hover:flex duration-500  gap-4 rounded  
             ${
               val.title == "About us"
                 ? " -bottom-[151px] z-10 -left-10"
                 : " -bottom-[293px] -left-12 "
             }
             `}
                >
                  {val.subLinks.map((subLink: SubLink) => (
                    <Link
                      key={subLink.href}
                      href={subLink.href}
                      className="w-max lg:hover:text-mm-green text-white rounded-[3px] duration-300 group-hover:block hidden"
                    >
                      {subLink.title}
                    </Link>
                  ))}
                </motion.div>
                <div className="absolute -bottom-6 -left-10 h-8 hidden group-hover:block w-36 bg-transparent " />
              </div>
            ) : (
              <Link
                className="font-medium text-sm capitalize outline-none lg:hover:text-mm-green text-white rounded-[3px] duration-300"
        ...
        <Button
          link="/contact"
          text="contact us"
          className="capitalize text-sm py-2 px-4 font-medium tracking-[5%] relative max-lg:hidden"
          key={"09+2"}
        />
        <svg
          className="lg:hidden"
